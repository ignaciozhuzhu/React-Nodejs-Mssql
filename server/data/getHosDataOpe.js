...
exports.getHosDataOpeTest2 = function(callback) {
	var db = require('../sqlserver/db');
	var str = "SELECT    *FROM         dbo.t_yy AS a INNER JOIN                      dbo.t_patient AS b ON a.cbrbh = b.cno INNER JOIN                      dbo.t_hosp AS c ON c.cno = a.hosp_no INNER JOIN                      dbo.t_employee AS d ON a.cysxm = d.cname AND (d.cTel1 IS NOT NULL OR                      d.cTel1 <> '') WHERE     (a.ldele = 0) and a.nid=16901 ";
	//var str = createyy() + "select * from ##yy where yyid=16901 order by yyid" //3.21当天
